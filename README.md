...
```
uz801_sms/
├── uz801_sms/                  # Python package
│   ├── __init__.py             # Public API exports
│   ├── __main__.py             # CLI entry point (python -m uz801_sms)
│   ├── manager.py              # UZ801 class — the main interface
│   ├── models.py               # SMS, SMSStatus, ReadState dataclasses
│   ├── parser.py               # Parses `content query` output
│   ├── adb_client.py           # ADB wrapper (auto-download, device detection)
│   ├── exceptions.py           # Custom exceptions
│   ├── constants.py            # USB IDs, IPs, content provider URIs
│   ├── README.md               # Package-level docs
│   └── HOW_IT_WORKS.md         # Deep dive: why AT commands don't work, etc.
├── examples/
│   ├── quickstart.py           # 30-second quickstart: read + monitor
│   ├── read_sms.py             # Read all/unread/inbox/sent SMS
│   ├── send_sms.py             # Send an SMS
│   ├── listen.py               # Block and listen for incoming SMS
│   ├── webhook.py              # Forward incoming SMS to an HTTP endpoint
│   └── multi_dongle.py         # Monitor multiple dongles simultaneously
...
The library also works as a command-line tool:

```bash
python -m uz801_sms setup                 # One-time: enable ADB debug mode
python -m uz801_sms info                  # Device info (IMEI, firmware, network)
python -m uz801_sms read                  # Read all SMS
python -m uz801_sms read --unread         # Read unread only
python -m uz801_sms send +972501234567 "Hello"  # Send SMS
python -m uz801_sms monitor               # Live monitor for new SMS
python -m uz801_sms delete 3              # Delete SMS by ID
python -m uz801_sms delete --all          # Delete all SMS
```
